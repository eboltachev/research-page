...
- `configs/routers.yml` — список карточек исследований (`path`, `url`, `password`, `name`, `description`, `sources`).
  ...
  - Запросы без префикса проекта (например, `/openapi.json` или `/register`) не маршрутизируются в upstream и возвращают `404`, чтобы исключить коллизии между разными приложениями.
