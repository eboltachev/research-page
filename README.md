...
- `configs/routers.yml` — список карточек исследований (`path`, `url`, `password`, `name`, `description`, `sources`).
  ...
  - Запросы без префикса проекта (например, `/openapi.json` или `/register`) маршрутизируются только при наличии `Referer` с валидным `/<path>/...`; иначе возвращается `404`, чтобы исключить коллизии между разными приложениями.
  - Для HTML/JS/CSS выполняется rewrite root-absolute ссылок (`/...`) в `/<path>/...`, чтобы SPA/dev-серверы (например Vite) корректно работали за path-prefix прокси.
